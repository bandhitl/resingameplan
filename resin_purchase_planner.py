...
FG_CAP = 500
SAFETY_MIN, SAFETY_MAX = 7, 10

# ── HISTORICAL PRICE (กราฟ) ─────────────
hist = pd.DataFrame({
...
def ai_trend():
    ...
    prompt = (
        ...
        "Summarise overall PVC import price trend in SE Asia for the next 3 months "
        "(choose only UP / DOWN / FLAT). List 3 key demand-supply factors.\n"
        'Return strict JSON e.g. {"trend":"UP","factors":["…","…"]}'
    )
    try:
...
def make_plan(df: pd.DataFrame):
    ...
    for i, row in df.iterrows():
        ...
        # 2) เติมให้ FG เต็ม 500 ถ้ายังไม่เต็มหลังขาย
        if fg + production - sales < FG_CAP:
            top_up = FG_CAP - (fg + production - sales)
            production += top_up
...
if st.button("🚀 Generate Plan"):
    ...
    st.download_button("⬇️ CSV", plan.to_csv(index=False).encode(),
                       file_name="resin_plan.csv", mime="text/csv")
